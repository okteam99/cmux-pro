...
(function () {
  ...
  const OUTLINE_COLLAPSED_KEY = "cmuxMdViewer.outline.collapsed";
  ...
  function buildOutline() {
    ...
    if (title) title.textContent = t("markdownViewer.outline.title", "Outline");
    ...
    headings.forEach((h) => {
      ...
      nav.appendChild(a);
      state.outlineLinks.push(a);
    });
    updateActiveOutlineItem();
  }

  function updateActiveOutlineItem() {
    if (!state.outlineLinks.length) return;
    let activeLink = state.outlineLinks[0];
    for (const link of state.outlineLinks) {
      const el = document.getElementById(link.dataset.targetId);
      if (!el) continue;
      if (el.getBoundingClientRect().top <= 12) {
        activeLink = link;
      } else {
        break;
      }
  ...
  function setOutlineCollapsed(collapsed, persist) {
    const outline = document.getElementById("outline");
    const toggle = document.getElementById("outlineToggle");
    if (!outline || !toggle) return;
    outline.dataset.collapsed = collapsed ? "true" : "false";
    toggle.setAttribute("aria-expanded", collapsed ? "false" : "true");
    const label = collapsed
      ? t("markdownViewer.outline.expand", "Expand outline")
      : t("markdownViewer.outline.collapse", "Collapse outline");
    ...
    if (persist) {
      try {
        ...
        // Storage may be unavailable in some configurations; ignore.
      }
    }
  }

  function initOutline() {
    if (state.outlineInited) return;
    const outline = document.getElementById("outline");
    const toggle = document.getElementById("outlineToggle");
    if (!outline || !toggle) return;
    state.outlineInited = true;

    let collapsed = false;
    try {
      collapsed = localStorage.getItem(OUTLINE_COLLAPSED_KEY) === "true";
    } catch (e) {
      // ignore
    }
    setOutlineCollapsed(collapsed, false);

    toggle.addEventListener("click", () => {
      const next = outline.dataset.collapsed !== "true";
      setOutlineCollapsed(next, true);
    });
