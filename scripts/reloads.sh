...
BUNDLE_ID="com.okteam99.cmuxpro.staging"
BASE_APP_NAME="cmux"
DERIVED_DATA=""
NAME_SET=0
BUNDLE_SET=0
...
if [[ -n "$TAG" ]]; then
  ...
  if [[ "$BUNDLE_SET" -eq 0 ]]; then
    BUNDLE_ID="com.okteam99.cmuxpro.staging.${TAG_ID}"
  fi
  if [[ "$DERIVED_SET" -eq 0 ]]; then
    DERIVED_DATA="/tmp/cmuxpro-staging-${TAG_SLUG}"
  fi
fi
...
if [[ -f "$INFO_PLIST" ]]; then
  ...
  CMUX_SOCKET_PATH_VALUE="/tmp/cmuxpro-${STAGING_SLUG}.sock"
