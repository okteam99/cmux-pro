...
launch_and_wait() {
  ...
  defaults write com.okteam99.cmuxpro.debug socketControlMode -string full >/dev/null 2>&1 || true
  ...
  SOCK=""
  for _ in {1..120}; do
    SOCK=$(ls -t /tmp/cmuxpro-debug*.sock /tmp/cmux*.sock 2>/dev/null | head -1 || true)
